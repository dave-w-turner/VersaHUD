...
namespace VersaHUD
{
    public partial class App : Application
    {
        ...
        public static async Task Log(string value)
        {
            ...
            try
            {
                string documentPath = FileSystem.Current.AppDataDirectory;
                string rollingLogPath = Path.Combine(documentPath, "VersaHUD_Continuous_SystemLog.txt");

                string formattedLogLine = $"[{DateTime.Now:yyyy-MM-dd HH:mm:ss}] {value}{Environment.NewLine}";

                lock (_fileWriteLock)
                {
                    File.AppendAllText(rollingLogPath, formattedLogLine, System.Text.Encoding.UTF8);
                }
            }
            catch
            {
